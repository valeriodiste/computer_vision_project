...
def print_model_evaluation_results(indexing_accuracy_evaluation_results, map_k_evaluation_results=None, recall_k_evaluation_results=None):
	''' Print the MAP@K and Recall@K evaluation results for the model '''
	# Print the indexing accuracy evaluation results
	if indexing_accuracy_evaluation_results is not None:
		additional_info = ""
		if "info" in indexing_accuracy_evaluation_results and indexing_accuracy_evaluation_results["info"] is not None and len(indexing_accuracy_evaluation_results["info"].keys()) > 0:
			additional_info = " (" + ", ".join([f"{key}: {value}" for key, value in indexing_accuracy_evaluation_results["info"].items()]) + ")"
		print(f"IA@{recall_k_evaluation_results['k_results']} for model \'{recall_k_evaluation_results['model_type']}\'{additional_info}: {indexing_accuracy_evaluation_results['accuracy']}")
	else:
		print(f"No indexing accuracy evaluation results for model \'{map_k_evaluation_results['model_type']}\'...")
	...
	if map_k_evaluation_results is not None:
		...
		print(f"MAP@{map_k_evaluation_results['k_results']} results for model \'{map_k_evaluation_results['model_type']}\'{additional_info}:")
		print(f"  > {map_k_evaluation_results['mean_average_precision']}")
		print(f"  Computed on {map_k_evaluation_results['n_images']} images")
		print(f"  Single images precision:")
		for image_id in map_k_evaluation_results['evaluated_images'].keys():
			print(f"    Image {image_id}: {map_k_evaluation_results['evaluated_images'][image_id]}")
	else:
		print(f"No MAP@K evaluation results for model \'{map_k_evaluation_results['model_type']}\'...")
	...
	if recall_k_evaluation_results is not None:
		...
		print(f"Recall@{recall_k_evaluation_results['k_results']} results results for model \'{recall_k_evaluation_results['model_type']}\'{additional_info}:")
		for i in range(len(recall_k_evaluation_results['recall_at_k_results'])):
			...
			print(f"    Computed for image {recall_k_evaluation_results['image_ids'][i]}")
		print(f"  Mean Recall@{recall_k_evaluation_results['k_results']}: {np.mean(recall_k_evaluation_results['recall_at_k_results'])}")
	else:
		print(f"No Recall@K evaluation results for model \'{map_k_evaluation_results['model_type']}\'...")
...
def get_image_data_as_base64(image_url_or_PIL_obj, image_max_size):
	'''
	...
	'''
	if isinstance(image_url_or_PIL_obj, str):
		# Load the image
		image = io.imread(image_url_or_PIL_obj)
		...
		image = np.array(image_url_or_PIL_obj)
		# Crop the image to a square aspect ratio if it is not already square
		downscaled_image = image
		if image.shape[1] > image.shape[0]:
			# Image is wider than tall, crop the sides
			...
			downscaled_image = image[crop_height:crop_height+image.shape[1], :]
		# Downscale the image to the maximum allowed size
		if downscaled_image.shape[0] > image_max_size or downscaled_image.shape[1] > image_max_size:
			# Downscale the image to the maximum allowed size
			downscaled_image = cv2.resize(downscaled_image, (image_max_size, image_max_size))
		# Convert the image to a base64 string
		image_base64 = base64.b64encode(cv2.imencode('.jpg', downscaled_image)[1]).decode()
